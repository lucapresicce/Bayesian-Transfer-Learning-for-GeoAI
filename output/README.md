...
* Section 5.1: `Predictive_coverage_simulation.RData`;
* Section 5.2: `replications_results.RData`;
* Section 6: `data_analysis.RData`;
...
**Note:** The output files `Predictive_coverage_simulation.RData` `replications_results.RData` and `data_analysis.RData` are **not included in this repository** because their size exceeds GitHub's 100 MB limit. However, they are **fully reproducible** by running the scripts [`Predictive_coverage_simulation.R`](../script/Predictive_coverage_simulation.R), [`Transfer_learning_simulation.R`](../script/Transfer_learning_simulation.R) and [`Vegetation_data_analysis.R`](../script/Vegetation_data_analysis.R), respectively.
Please be aware that these scripts may take **long times to execute**, depending on your system’s resources. If needed, the original `Predictive_coverage_simulation.RData`, `replications_results.RData`, or `data_analysis.RData` file can be provided upon request.
...
* Section 6: [`NDVI500_map.png`](./NDVI500_map.png), [`RR500_map.png`](./RR500_map.png);

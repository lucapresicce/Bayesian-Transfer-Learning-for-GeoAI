...
apply_continent_mask <- function(df, raster_mask) {
  ...
  mask_values <- extract(raster_mask, coords)
...
# Combina i grafici in un'unica figura
width <- 360*5
height <- 360*2
pointsize <- 12
png("output/dataanalysis_multivariate.png", width = width, height = height, pointsize = pointsize, family = "sans")
grid.arrange(NDVI_plot, RR_plot, NDVI_test_plot, RR_test_plot, uc_Y1, uc_Y2, NDVI_hat_plot, RR_hat_plot, nrow = 2, ncol = 4)
dev.off()
...
results <- list("time"    = elapsed_times,
                "comb"    = comb_bps,
                "pred"    = predictions,
                "metrics" = c("RMSPE_bps" = rmspe_Y, "RMPSE_naive" = rmspe_naive, "CI_len" = CI_avlen_masmk, "ECoverage" = coverage_Y, "MAPE_bps" = mape_Y, "MAPE_naive" = mape_naive))

rm(list = ls()[which(!(ls() %in% c("results")))])
save.image(file = "output/dataanalysis_multivariate.RData")

...
for (b in 1:BB) {
  ...
  posterior_bps <- cbind(
    matrix(out$posterior$sigma, nrow = R, byrow = T)[,c(1,2,5,3,6,9)],
    matrix(out$posterior$beta, nrow = R, byrow = T))
  ...
  replication[[4]] <- list("posterior"  = posterior_bps,
                           "prediction" = out$predictive$Yu,
                           "time" = new_time,
                           "weights" = matrix(out$weights_global))
...
metric_names <- c("Empirical Bias", "Coverage", "Std. Dev.")
metric_list  <- list(
...
plot_list <- lapply(metric_names, function(metric) {
  ...
  ylim_val <- switch(metric,
                     "Empirical Bias" = c(-2, 1),
                     "Coverage"       = c(0, 1),
                     "Std. Dev."      = c(0, max(df_sub$Value) * 1.1)
  )
  
  ggplot(df_sub, aes(x = Parameter, y = Value, fill = Setting)) +
    geom_col(position = position_dodge2(width = 0.1), width = 0.5) +
    scale_fill_manual(values = setting_colors) +
    scale_x_discrete(labels = param_labels) +
    labs(x = NULL, y = metric, fill = "Setting") +
...
png("output/TL_post.png", width = 14, height = 8, units = "in", res = 300, family = "sans")
print(final_plot)
dev.off()

metric_names <- c("MSPE", "Pred Int Width", "Absolute Bias", "Variance")
array_list   <- list(pred_rmpse_array, pred_length_array, abs(pred_bias_array), pred_std_array^2)
...
png("output/TL_pred.png", width = 14, height = 8, units = "in", res = 300, family = "sans")
print(final_plot2)
dev.off()
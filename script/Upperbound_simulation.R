...
calculate_post <- function(Z, K = 5, sqJ = 2) {
...
df <- data_gen(n = n)
post_weights <- calculate_post(Z = df, K = 5, sqJ = 2)
...
ExpVal_jk <- function(k, j, J, L, data, alpha_seq, phi_seq) {
...
KL_ub <- function(n, K, J, M = 1) {
  ...
  for (k in 1:K) {
    ...
    for (j in 1:J) {
      ...
      E_JK[j, k] <- ExpVal_jk(k = k, j = j, J = J, L = 100, data = data, alpha_seq = alpha_seq, phi_seq = phi_seq)
...
K_plot <- ggplot(df_K, aes(x, y)) +
  ...
  labs(title = "",
       x = "K (number of subsets)", y = "KL divergence") +
  theme(
...
J_plot <- ggplot(df_J, aes(x, y)) +
  ...
  theme_minimal(base_size = 14) +
  labs(title = "",
       x = "J (number of model)", y = "KL divergence") +
  theme(
    axis.text  = element_text(size = 16, face = "bold"),
    axis.title = element_text(size = 16, face = "bold")
  )
...
png("upperbound_sim.png", width = 12, height = 4, units = "in", res = 300, family = "sans")
print(K_plot | J_plot)
dev.off()

...
# x <- cbind(1, matrix(as.matrix(X[subsample, 1]), ncol = 1))
...
# x_u <- cbind(1, matrix(as.matrix(X_U[subsampleu, 1]), ncol = 1))
...
# training and timing (for each competitor)
...
preds_per_model$automl <- h2o.predict(models_list$automl@leader, newdata_h2o)
preds_per_model$deeplearning <- h2o.predict(models_list$deeplearning, newdata_h2o)
preds_per_model$drf <- h2o.predict(models_list$drf, newdata_h2o)
preds_per_model$xgboost <- h2o.predict(models_list$xgboost, newdata_h2o)

# combine predictions into one data frame
pred_results_df <- list()
for (model in names(preds_per_model)) {
  pred_df <- as.data.frame(preds_per_model[[model]])
  colnames(pred_df) <- model
  pred_results_df[[model]] <- pred_df
}
...
for (model in names(preds_per_model)) {
    y_pred <- as.matrix(preds_per_model[[model]])
...
comparison_table <- merge(timing_log, rmspe_log, by = "model", all = TRUE)
...
height <- 360*2
pointsize <- 12
# png("output/dataanalysis_univariate.png", width = width, height = height, pointsize = pointsize, family = "sans")
...
competitor_results <- list("time"   = timing_log,
                           "rmspe"  = rmspe_log,
                           "models" = models_list,
                           "pred"   = preds_per_model,
                           "table"  = comparison_table)


rm(list = ls()[which(!(ls() %in% c("results", "competitor_results")))])
save.image(file = "review_DataAppl.RData")

# to read results
# load("review_DataAppl.RData")
# competitor_results$time

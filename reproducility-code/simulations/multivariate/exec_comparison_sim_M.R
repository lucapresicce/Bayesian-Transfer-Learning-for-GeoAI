...
rm(list = ls())
gc()
setwd(".../reproducibility-code/simulations/multivariate")
# Packages --------------------------------------------------------------------
library(ASMK)
library(Rcpp)
...
# Multivariate ASMK parallel fit -------------------------------------------------------
...
cat("Empirical average MASMK coverage for Responses:", round(mean(coverage_Y), 3))
...
# collecting
posterior_asmk <- cbind(t(sapply(1:R, function(r)matrix(beta_smp[,,r]))),
                        t(sapply(1:R, function(r)matrix(sigma_smp[,,r])))[,-3],
                        cbind(rep(post_mean_hyp[2], R), rep(post_mean_hyp[2], R)))
colnames(posterior_asmk) <- c("beta[1,1]", "beta[2,1]", "beta[1,2]", "beta[2,2]", "Sigma[1,1]", "Sigma[2,1]", "Sigma[2,2]", "phi[1]", "phi[2]")

# plotting asmk
bayesplot_theme_set(theme_default(base_size = 14, base_family = "sans"))
color_scheme_set("red")
post_dens_asmk <- mcmc_areas(posterior_asmk,
                             prob = 0.95,
                             point_est = "mean") +
  ggtitle("Posterior distributions MASMK",
          "with posterior means, true values, and 95% credible intervals") +
  scale_y_discrete(labels = c(expression(beta[list(1,1)]), expression(beta[list(2,1)]), expression(beta[list(1,2)]), expression(beta[list(2,2)]),
...
post_int_asmk <- mcmc_recover_intervals(posterior_asmk, true_par,
                                        prob = 0.89,
                                        prob_outer = 0.95,
                                        point_est = "mean",
                                        size = 4,
  ...
  scale_x_discrete(labels = c(expression(beta[list(1,1)]), expression(beta[list(2,1)]), expression(beta[list(1,2)]), expression(beta[list(2,2)]),
                              expression(phi[1]), expression(phi[2]),
                              expression(Sigma[list(1,1)]), expression(Sigma[list(2,1)]), expression(Sigma[list(2,2)]))) +
  ggtitle("Credible intervals MASMK",
...
png("post_asmk_5_500.png", width = width, height = height, pointsize = pointsize, family = "sans")
# win.graph()
cowplot::plot_grid(post_dens_asmk, post_int_asmk)
dev.off()
...
bayesplot_theme_set(theme_default(base_size = 14, base_family = "sans"))
color_scheme_set("red")
post_dens_smk <- mcmc_areas(posterior_smk,
                            prob = 0.95,
                            point_est = "mean") +
  ggtitle("Posterior distributions MSMK",
          "with posterior means, true values, and 95% credible intervals") +
  scale_y_discrete(labels = c(expression(beta[list(1,1)]), expression(beta[list(2,1)]), expression(beta[list(1,2)]), expression(beta[list(2,2)]),
                              expression(Sigma[list(1,1)]), expression(Sigma[list(2,1)]), expression(Sigma[list(2,2)]),
                              expression(phi[1]), expression(phi[2])))
...
png("post_smk_5_500.png", width = width, height = height, pointsize = pointsize, family = "sans")
# win.graph()
cowplot::plot_grid(post_dens_smk, post_int_smk)
dev.off()
...
png("post_sim_5_500.png", width = width, height = height, pointsize = pointsize, family = "sans")
# win.graph()
cowplot::plot_grid(post_int_asmk, post_int_smk)
dev.off()
...
cat("minutes elapsed for MASMK fully model-based uncertainty quantification : \n"); round(elapsed_times/60, 2)
...
surf.W1 <- MBA::mba.surf(cbind(crd_s, W[,1]), no.X = 500, no.Y = 500,
                         exten = TRUE, sp = TRUE, h = h)$xyz.est
surf.W2 <- MBA::mba.surf(cbind(crd_s, W[,2]), no.X = 500, no.Y = 500,
                         exten = TRUE, sp = TRUE, h = h)$xyz.est
surf.brks.w <- classIntervals(c(surf.W1$z, surf.W2$z), 100, 'pretty')$brks
...
zlimw <- range(c(surf.W1$z, surf.W2$z))
...
# Plotting
png("comp_sim_5_500.png", width = width, height = height, pointsize = pointsize, family = "sans")
# win.graph()
par(mfrow = c(2, 2))
iw1 <- as.image.SpatialGridDataFrame(surf.W1)
plot(crd, type="n", cex=0.5, xlim=xlim, axes=FALSE, ylab="Northing", xlab="Easting",
     main="1st Spatial process")
axis(2, las=1)
axis(1)
image.plot(iw1, add=TRUE, col=rev(col.pal(length(surf.brks.w)-1)), zlim=zlimw)

iw2 <- as.image.SpatialGridDataFrame(surf.W2)
plot(crd, type="n", cex=0.5, xlim=xlim, axes=FALSE, ylab="Northing", xlab="Easting",
     main="2nd Spatial process")
axis(2, las=1)
axis(1)
image.plot(iw2, add=TRUE, col=rev(col.pal(length(surf.brks.w)-1)), zlim=zlimw)
...
# Plotting MASMK results --------------------------------------------------------

# surfaces interpolation
h <- 12

surf.W1p <- MBA::mba.surf(cbind(crd_u, post_mean_W[,1]), no.X = 500, no.Y = 500,
                          exten = TRUE, sp = TRUE, h = h)$xyz.est
surf.W2p <- MBA::mba.surf(cbind(crd_u, post_mean_W[,2]), no.X = 500, no.Y = 500,
                          exten = TRUE, sp = TRUE, h = h)$xyz.est
surf.brks.w <- classIntervals(c(surf.W1p$z, surf.W2p$z), 100, 'pretty')$brks
...
zlimwp <- range(c(surf.W1p$z, surf.W2p$z))
...
# Plotting
png("comp_res_5_500.png", width = width, height = height, pointsize = pointsize, family = "sans")
# win.graph()
par(mfrow = c(2, 2))

iw1p <- as.image.SpatialGridDataFrame(surf.W1p)
plot(crd, type="n", cex=0.5, xlim=xlim, axes=F, ylab="Northing", xlab="Easting")
title(main="1st Predicted (Posterior Mean) Spatial process")
mtext(side = 3, paste("RMSPE :", round(rmspe_W[1], 3)))
axis(2, las=1)
axis(1)
image.plot(iw1p, add=TRUE, col=rev(col.pal(length(surf.brks.w)-1)), zlim=zlimwp)

iw2p <- as.image.SpatialGridDataFrame(surf.W2p)
plot(crd, type="n", cex=0.5, xlim=xlim, axes=F, ylab="Northing", xlab="Easting")
title(main="2nd Predicted (Posterior Mean) Spatial process")
mtext(side = 3, paste("RMSPE :", round(rmspe_W[2], 3)))
axis(2, las=1)
axis(1)
image.plot(iw2p, add=TRUE, col=rev(col.pal(length(surf.brks.w)-1)), zlim=zlimwp)

iy1p <- as.image.SpatialGridDataFrame(surf.Y1p)
plot(crd, type="n", cex=0.5, xlim=xlim, axes=F, ylab="Northing", xlab="Easting")
title(main="1st Predicted (Posterior Mean) Response")
...
plot(crd, type="n", cex=0.5, xlim=xlim, axes=F, ylab="Northing", xlab="Easting")
title(main="2nd Predicted (Posterior Mean) Response")
...
#        title = "Response Credible Intervals for new locations",
...
#         axis.line = element_line(colour = "black"))
#
# png("comp_cover_5_500.png", width = width, height = height, pointsize = pointsize, family = "sans")
# win.graph()
# cowplot::plot_grid(uc_Y1, uc_Y2, nrow = 1)
# dev.off()
...
# Plotting
png("comp_comp_5_500.png", width = width, height = height, pointsize = pointsize, family = "sans")
# win.graph()
par(mfrow = c(2, 3))
...
plot(crd, type="n", cex=0.5, xlim=xlim, axes=F, ylab="Northing", xlab="Easting")
title(main="MASMK Prediction (Posterior Mean) for 1st Response")
mtext(side = 3, paste("RMSPE :", round(rmspe_Y[1], 3)))
axis(2, las=1)
axis(1)
image.plot(iy1p, add=TRUE, col=rev(col.pal(length(surf.brks.y)-1)), zlim=zlimyp)
...
plot(crd, type="n", cex=0.5, xlim=xlim, axes=F, ylab="Northing", xlab="Easting")
title(main="MASMK Prediction (Posterior Mean) for 2nd Response")
mtext(side = 3, paste("RMSPE :", round(rmspe_Y[2], 3)))
axis(2, las=1)
axis(1)
image.plot(iy2p, add=TRUE, col=rev(col.pal(length(surf.brks.y)-1)), zlim=zlimyp)
...
# graphical UC comparison for Y (MASMK)
...
# Create the ggplot
uc_Y1asmk <- ggplot(df, aes(x = x_ax, y = Yu_ord)) +
  geom_point(pch = 18, size = 3.5, col = "#1A85FF") +
  geom_errorbar(aes(ymin = CI_Y_lower, ymax = CI_Y_upper),
                width = 0.05,
                linetype = "dashed",
  ...
  ylim(range(c(df$CI_Y_lower, df$CI_Y_upper))) +
  labs(x = "Ordered locations", y = "Response values",
       title = "MASMK 1st Response Credible Intervals for new locations",
       subtitle = paste("Empirical coverage :", round(coverage_Y[1], 3))) +
  geom_point(aes(y = Ymap_ord), pch = 15, size = 1.5, col = "#D41159") +
  theme(panel.background = element_blank(),
        panel.grid.major = element_blank(),
...
# Create the ggplot
uc_Y2asmk <- ggplot(df, aes(x = x_ax, y = Yu_ord)) +
  geom_point(pch = 18, size = 3.5, col = "#1A85FF") +
  geom_errorbar(aes(ymin = CI_Y_lower, ymax = CI_Y_upper),
                width = 0.05,
                linetype = "dashed",
  ...
  ylim(range(c(df$CI_Y_lower, df$CI_Y_upper))) +
  labs(x = "Ordered locations", y = "Response values",
       title = "MASMK 2nd Response Credible Intervals for new locations",
       subtitle = paste("Empirical coverage :", round(coverage_Y[2], 3))) +
  geom_point(aes(y = Ymap_ord), pch = 15, size = 1.5, col = "#D41159") +
  theme(panel.background = element_blank(),
        panel.grid.major = element_blank(),
...
png("comp_UCcomp_5_500.png", width = width, height = height, pointsize = pointsize, family = "sans")
# win.graph()
cowplot::plot_grid(uc_Y1asmk, uc_Y2asmk, uc_Y1smk, uc_Y2smk)
dev.off()
...
results <- list("ASMK" = list("time"    = elapsed_times,
                              "fit"     = obj_fit,
                              "comb"    = comb_bps,
                              "pred"    = predictions,
                              "post"    = list(beta_smp, sigma_smp),
                              "metrics" = c("RMSPE" = rmspe_Y, "CI_len" = CI_avlen_masmk, "ECoverage" = coverage_Y)),
                "SMK" = list("time"    = elapsed_smk,
                             "fit"     = obj,
                             "comb"    = wts,
                             "pred"    = list(Y.med, Y.lower_qnt, Y.upper_qnt),
                             "post"    = posterior_smk,
                             "metrics" = c("RMSPE" = rmspe_smk, "CI_len" = CI_avlen_smk, "ECoverage" = coverage_smk)))

rm(list = ls()[which(!(ls() %in% c("results")))])
save.image(file = "COMPARISON_5_500.RData")

# load("COMPARISON_5_500.RData")
# res_5_500_M <- results
# rm(list = "results")
# gc()
#
# # timing
# cat("minutes elapsed for MASMK fully model-based uncertainty quantification : \n"); round(res_5_500_M$ASMK$time/60, 2)
# cat("minutes elapsed for MSMK fully model-based uncertainty quantification : \n"); round(res_5_500_M$SMK$time/60, 2)
...
png("post_asmk_5_1000.png", width = width, height = height, pointsize = pointsize, family = "sans")
# win.graph()
cowplot::plot_grid(post_dens_asmk, post_int_asmk)
dev.off()
...
png("post_smk_5_1000.png", width = width, height = height, pointsize = pointsize, family = "sans")
# win.graph()
cowplot::plot_grid(post_dens_smk, post_int_smk)
dev.off()
...
png("post_sim_5_1000.png", width = width, height = height, pointsize = pointsize, family = "sans")
# win.graph()
cowplot::plot_grid(post_int_asmk, post_int_smk)
dev.off()
...
# Plotting
png("comp_sim_5_1000.png", width = width, height = height, pointsize = pointsize, family = "sans")
# win.graph()
par(mfrow = c(2, 2))
iw1 <- as.image.SpatialGridDataFrame(surf.W1)
plot(crd, type="n", cex=0.5, xlim=xlim, axes=FALSE, ylab="Northing", xlab="Easting",
...
# Plotting
png("comp_res_5_1000.png", width = width, height = height, pointsize = pointsize, family = "sans")
# win.graph()
par(mfrow = c(2, 2))
...
# Plotting
png("comp_comp_5_1000.png", width = width, height = height, pointsize = pointsize, family = "sans")
# win.graph()
par(mfrow = c(2, 3))
...
png("comp_UCcomp_5_1000.png", width = width, height = height, pointsize = pointsize, family = "sans")
# win.graph()
cowplot::plot_grid(uc_Y1asmk, uc_Y2asmk, uc_Y1smk, uc_Y2smk)
dev.off()
...
rm(list = ls()[which(!(ls() %in% c("results")))])
save.image(file = "COMPARISON_5_1000.RData")

# load("COMPARISON_5_1000.RData")
# res_5_1000_M <- results
# rm(list = "results")
# gc()
#
# # timing
# cat("minutes elapsed for MASMK fully model-based uncertainty quantification : \n"); round(res_5_1000_M$ASMK$time/60, 2)
# cat("minutes elapsed for MSMK fully model-based uncertainty quantification : \n"); round(res_5_1000_M$SMK$time/60, 2)
...
png("post_asmk_10_500.png", width = width, height = height, pointsize = pointsize, family = "sans")
# win.graph()
cowplot::plot_grid(post_dens_asmk, post_int_asmk)
dev.off()
...
png("post_smk_10_500.png", width = width, height = height, pointsize = pointsize, family = "sans")
# win.graph()
cowplot::plot_grid(post_dens_smk, post_int_smk)
dev.off()
...
png("post_sim_10_500.png", width = width, height = height, pointsize = pointsize, family = "sans")
# win.graph()
cowplot::plot_grid(post_int_asmk, post_int_smk)
dev.off()
...
# Plotting
png("comp_sim_10_500.png", width = width, height = height, pointsize = pointsize, family = "sans")
# win.graph()
par(mfrow = c(2, 2))
iw1 <- as.image.SpatialGridDataFrame(surf.W1)
plot(crd, type="n", cex=0.5, xlim=xlim, axes=FALSE, ylab="Northing", xlab="Easting",
...
# Plotting
png("comp_res_10_500.png", width = width, height = height, pointsize = pointsize, family = "sans")
# win.graph()
par(mfrow = c(2, 2))
...
# Plotting
png("comp_comp_10_500.png", width = width, height = height, pointsize = pointsize, family = "sans")
# win.graph()
par(mfrow = c(2, 3))
...
png("comp_UCcomp_10_500.png", width = width, height = height, pointsize = pointsize, family = "sans")
# win.graph()
cowplot::plot_grid(uc_Y1asmk, uc_Y2asmk, uc_Y1smk, uc_Y2smk)
dev.off()
...
rm(list = ls()[which(!(ls() %in% c("results")))])
save.image(file = "COMPARISON_10_500.RData")

# load("COMPARISON_10_500.RData")
# res_10_500_M <- results
# rm(list = "results")
# gc()
#
# # timing
# cat("minutes elapsed for MASMK fully model-based uncertainty quantification : \n"); round(res_10_500_M$ASMK$time/60, 2)
# cat("minutes elapsed for MSMK fully model-based uncertainty quantification : \n"); round(res_10_500_M$SMK$time/60, 2)
...
png("post_asmk_10_1000.png", width = width, height = height, pointsize = pointsize, family = "sans")
# win.graph()
cowplot::plot_grid(post_dens_asmk, post_int_asmk)
dev.off()
...
png("post_smk_10_1000.png", width = width, height = height, pointsize = pointsize, family = "sans")
# win.graph()
cowplot::plot_grid(post_dens_smk, post_int_smk)
dev.off()
...
png("post_sim_10_1000.png", width = width, height = height, pointsize = pointsize, family = "sans")
# win.graph()
cowplot::plot_grid(post_int_asmk, post_int_smk)
dev.off()
...
# Plotting
png("comp_sim_10_1000.png", width = width, height = height, pointsize = pointsize, family = "sans")
# win.graph()
par(mfrow = c(2, 2))
iw1 <- as.image.SpatialGridDataFrame(surf.W1)
plot(crd, type="n", cex=0.5, xlim=xlim, axes=FALSE, ylab="Northing", xlab="Easting",
...
# Plotting
png("comp_res_10_1000.png", width = width, height = height, pointsize = pointsize, family = "sans")
# win.graph()
par(mfrow = c(2, 2))
...
# Plotting
png("comp_comp_10_1000.png", width = width, height = height, pointsize = pointsize, family = "sans")
# win.graph()
par(mfrow = c(2, 3))
...
png("comp_UCcomp_10_1000.png", width = width, height = height, pointsize = pointsize, family = "sans")
# win.graph()
cowplot::plot_grid(uc_Y1asmk, uc_Y2asmk, uc_Y1smk, uc_Y2smk)
dev.off()
...
rm(list = ls()[which(!(ls() %in% c("results")))])
save.image(file = "COMPARISON_10_1000.RData")

# load("COMPARISON_10_1000.RData")
# res_10_1000_M <- results
# rm(list = "results")
# gc()
#
# # timing
# cat("minutes elapsed for MASMK fully model-based uncertainty quantification : \n"); round(res_10_1000_M$ASMK$time/60, 2)
# cat("minutes elapsed for MSMK fully model-based uncertainty quantification : \n"); round(res_10_1000_M$SMK$time/60, 2)

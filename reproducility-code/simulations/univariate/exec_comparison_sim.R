#####################################################################################################################################################
## ASMK COMPARISON - 5K - 500 ############################################
rm(list = ls())
gc()
setwd(".../reproducibility-code/simulations/univariate")
# Packages --------------------------------------------------------------------
library(ASMK)
library(Rcpp)
...
# asmk
# smp <- sapply(1:M, function(r){post_smp_asmk[[r]]})
...
surf.Y <- MBA::mba.surf(cbind(crd_s, Y), no.X = 500, no.Y = 500,
                        exten = TRUE, sp = TRUE, h = h)$xyz.est
surf.brks <- classIntervals(c(surf.W$z, surf.Y$z), 100, 'pretty')$brks
col.pal <- colorRampPalette(brewer.pal(11,'RdBu')[11:1])
xlim <- c(0, 1)

zlim <- range(c(surf.W$z, surf.Y$z))
...
surf.Yp <- MBA::mba.surf(cbind(crd_u, post_mean_Y), no.X = 500, no.Y = 500,
                         exten = TRUE, sp = TRUE, h = h)$xyz.est
zlimp <- range(c(surf.Wp$z, surf.Yp$z))
...
# graphical UC for W (ordered)
ord_w <- order(W_u[-(1:m)])
df <- data.frame(
  x_ax = 1:u,
  Wu_ord = W_u[-(1:m)][ord_w],
  CI_W_lower = post_qnt_W[,-(1:m)][, ord_w][1,],
  CI_W_upper = post_qnt_W[,-(1:m)][, ord_w][2,],
  Wmap_ord = post_mean_W[-(1:m)][ord_w])
# Create the ggplot
uc_W <- ggplot(df, aes(x = x_ax, y = Wu_ord)) +
  geom_point(pch = 18, size = 2.5, col = "#1A85FF") +
  geom_errorbar(aes(ymin = CI_W_lower, ymax = CI_W_upper),
                width = 0.75,
                linetype = "dashed",
                linewidth = 0.25,
                color = "#D41159") +
  ylim(range(c(df$CI_W_lower, df$CI_W_upper))) +
  labs(x = "Ordered locations", y = "Spatial process values",
       title = "Response Credible Intervals for new locations",
       subtitle = paste("Empirical coverage :", round(coverage_W, 3))) +
  geom_point(aes(y = Wmap_ord), pch = 15, size = 1.5, col = "#D41159") +
...
ord_y <- order(Y_u[-(1:m)])
# plot_ind <- sample(1:u, 750)
df <- data.frame(
  x_ax = (1:u), #[plot_ind],
  Yu_ord = Y_u[-(1:m)][ord_y], #[plot_ind],
  CI_Y_lower = post_qnt_Y[,-(1:m)][, ord_y][1,], #[plot_ind],
  CI_Y_upper = post_qnt_Y[,-(1:m)][, ord_y][2,], #[plot_ind],
  Ymap_ord = post_mean_Y[-(1:m)][ord_y]) #[plot_ind])
# Create the ggplot
...
# win.graph()
cowplot::plot_grid(uc_W, uc_Y)
dev.off()

# Plotting COMP results --------------------------------------------------------
...
image.plot(iy, add=TRUE, col=rev(col.pal(length(surf.brks)-1)), zlim=zlim)
plot(crd, type="n", cex=0.5, xlim=xlim, axes=F, ylab="Northing", xlab="Easting")
title(main="ASMK Prediction (Posterior Mean) for Response")
mtext(side = 3, paste("RMSPE :", round(rmspe_Y, 3)))
axis(2, las=1)
axis(1)
image.plot(iyp, add=TRUE, col=rev(col.pal(length(surf.brks)-1)), zlim=zlimp)
...
# graphical UC comparison for Y (ASMK vs SMK - ordered)
ord_y <- order(Y_u[-(1:m)])
df <- data.frame(
  x_ax = 1:u,
...
# Create the ggplot
uc_Yasmk <- ggplot(df, aes(x = x_ax, y = Yu_ord)) +
  geom_point(pch = 18, size = 2.5, col = "#1A85FF") +
  geom_errorbar(aes(ymin = CI_Y_lower, ymax = CI_Y_upper),
                width = 1,
                linetype = "dashed",
  ...
  ylim(range(c(df$CI_Y_lower, df$CI_Y_upper))) +
  labs(x = "Ordered locations", y = "Response values",
       title = "ASMK Response Credible Intervals for new locations",
       subtitle = paste("Empirical coverage :", round(coverage_Y, 3))) +
  geom_point(aes(y = Ymap_ord), pch = 15, size = 1.5, col = "#D41159") +
  theme(panel.background = element_blank(),
        panel.grid.major = element_blank(),
...
# win.graph()
cowplot::plot_grid(uc_Yasmk, uc_Ysmk)
dev.off()
...
png("comp_cover_5_1000.png", width = width, height = height, pointsize = pointsize, family = "sans")
# win.graph()
cowplot::plot_grid(uc_W, uc_Y)
dev.off()
...
png("comp_cover_10_500.png", width = width, height = height, pointsize = pointsize, family = "sans")
# win.graph()
cowplot::plot_grid(uc_W, uc_Y)
dev.off()
...
png("comp_cover_10_1000.png", width = width, height = height, pointsize = pointsize, family = "sans")
# win.graph()
cowplot::plot_grid(uc_W, uc_Y)
dev.off()
...
# timing
cat("ASMK timing : \n"); round(res_5_500$ASMK$time/60, 2)
cat("SMK timing : \n"); round(res_5_500$SMK$time/60, 2)
cat("Relative timing ratio SMK over ASMK : \n"); round(as.numeric(as.numeric(round(res_5_500$SMK$time/60, 2))[4] / round(res_5_500$ASMK$time/60, 2)[4]), 2)
...
posterior_asmk <- t(res_5_500$ASMK$post)
posterior_smk <- cbind(res_5_500$SMK$post[,5], res_5_500$SMK$post[,1:4])
colnames(posterior_asmk) <- colnames(posterior_smk) <- c("sigma^2", "beta[1]", "beta[2]", "beta[3]", "beta[4]")
...
mcmc_areas(posterior_asmk,
           prob = 0.95,
           point_est = "mean") + 
  ggtitle("Posterior distributions ASMK",
          "with posterior means, true values, and 95% credible intervals") +
  scale_y_discrete(labels = c(expression(sigma^2), expression(beta[1]), expression(beta[2]), expression(beta[3]), expression(beta[4])))
...
post_int_asmk <- mcmc_recover_intervals(posterior_asmk, true_par,
  ...
  ggtitle("Credible intervals ASMK",
...
bayesplot_theme_set(theme_default(base_size = 14, base_family = "sans"))
color_scheme_set("red")
mcmc_areas(posterior_smk,
           prob = 0.95,
           point_est = "mean") + 
  ggtitle("Posterior distributions SMK",
          "with posterior means, true values, and 95% credible intervals") +
  # geom_vline(xintercept = true_par, linetype = "dashed", color = "red") +
  scale_y_discrete(labels = c(expression(sigma^2), expression(beta[1]), expression(beta[2]), expression(beta[3]), expression(beta[4])))
...
# timing
cat("ASMK timing : \n"); round(res_5_1000$ASMK$time/60, 2)
cat("SMK timing : \n"); round(res_5_1000$SMK$time/60, 2)
cat("Relative timing ratio SMK over ASMK : \n"); round(as.numeric(as.numeric(round(res_5_1000$SMK$time/60, 2))[4] / round(res_5_1000$ASMK$time/60, 2)[4]), 2)
...
# posterior samples collection
posterior_asmk <- t(res_5_1000$ASMK$post)
posterior_smk <- cbind(res_5_1000$SMK$post[,5], res_5_1000$SMK$post[,1:4])
colnames(posterior_asmk) <- colnames(posterior_smk) <- c("sigma^2", "beta[1]", "beta[2]", "beta[3]", "beta[4]")
...
# timing
cat("ASMK timing : \n"); round(res_10_500$ASMK$time/60, 2)
cat("SMK timing : \n"); round(res_10_500$SMK$time/60, 2)
cat("Relative timing ratio SMK over ASMK : \n"); round(as.numeric(as.numeric(round(res_10_500$SMK$time/60, 2))[4] / round(res_10_500$ASMK$time/60, 2)[4]), 2)
...
# posterior samples collection
posterior_asmk <- t(res_10_500$ASMK$post)
posterior_smk <- cbind(res_10_500$SMK$post[,5], res_10_500$SMK$post[,1:4])
colnames(posterior_asmk) <- colnames(posterior_smk) <- c("sigma^2", "beta[1]", "beta[2]", "beta[3]", "beta[4]")
...
# timing
cat("ASMK timing : \n"); round(res_10_1000$ASMK$time/60, 2)
cat("SMK timing : \n"); round(res_10_1000$SMK$time/60, 2)
cat("Relative timing ratio SMK over ASMK : \n"); round(as.numeric(as.numeric(round(res_10_1000$SMK$time/60, 2))[4] / round(res_10_1000$ASMK$time/60, 2)[4]), 2)
...
# posterior samples collection
posterior_asmk <- t(res_10_1000$ASMK$post)
posterior_smk <- cbind(res_10_1000$SMK$post[,5], res_10_1000$SMK$post[,1:4])
colnames(posterior_asmk) <- colnames(posterior_smk) <- c("sigma^2", "beta[1]", "beta[2]", "beta[3]", "beta[4]")

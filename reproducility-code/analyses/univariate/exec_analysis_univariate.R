...
rm(list = ls())
gc()
setwd(".../reproducibility-code/analyses/univariate")
...
height <- 360
pointsize <- 12
png("dataanalysis_univariate_pred.png", width = width, height = height, pointsize = pointsize, family = "sans")
cowplot::plot_grid(combined_test, combined_pred, nrow = 1)
dev.off()
...
height <- 360*2
pointsize <- 12
png("dataanalysis_univariate_all.png", width = width, height = height, pointsize = pointsize, family = "sans")
...
save.image(file = "datanalysis_univariate.RData")
# load("datanalysis_univariate.RData")
